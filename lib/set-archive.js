...
export function listFlowSets(now = Date.now()) {
  if (!fs.existsSync(FLOW)) return [];
  const rows = [];
  for (const name of fs.readdirSync(FLOW)) {
    const manifestPath = path.join(FLOW, name, "manifest.json");
...
function setDir(id) {
  ...
  if (!row) return null;
  const dir = path.join(FLOW, row.folder);
