...
import { trialEndFrom } from "./account-rights.js";
...
function normalize(raw) {
  ...
  return {
    ...
    trialEndsAt: String(raw.trialEndsAt || ""),
    trialNoticeAt: String(raw.trialNoticeAt || ""),
  };
}
...
const DELETED_FILE = process.env.FA_DELETED_PATH || "/tmp/fa-deleted-accounts.json";

function deletionEntry(rec, action, locale) {
  return {
    at: new Date().toISOString(),
    email: rec.email || "",
    country: String(rec.country || "").slice(0, 8),
    locale: String(locale || "").slice(0, 8),
    action: action === "withdrawal" ? "withdrawal" : "delete",
    mailConsent: false,
...
async function rememberDeletion(entry) {
  const db = await getRedis();
  if (db) {
    await db.lpush("fa:deleted-accounts", entry);
    await db.ltrim("fa:deleted-accounts", 0, 5000);
    return;
  }
  let rows = [];
  try { rows = JSON.parse(fs.readFileSync(DELETED_FILE, "utf8")); } catch { rows = []; }
  if (!Array.isArray(rows)) rows = [];
  rows.unshift(entry);
  try { fs.writeFileSync(DELETED_FILE, JSON.stringify(rows.slice(0, 5000))); } catch { /* ignore */ }
}

async function forgetRecord(rec) {
  const db = await getRedis();
  if (db) {
    if (rec.email) {
      await db.del(`fa:profile:email:${rec.email}`);
      try { await db.srem(EMAILS_KEY, rec.email); } catch { /* set may be absent */ }
    }
    if (rec.loginName) await db.del(`fa:profile:login:${rec.loginName}`);
    await db.del(`fa:profile:id:${rec.id}`);
    return;
  }
  ensureLoaded();
  g.__faProfiles.list = g.__faProfiles.list.filter((p) => p.id !== rec.id);
  persist();
}

export async function setMailConsent(id, on) {
  const rec = await findProfileById(id);
  if (!rec) throw codeError("unauthorized");
  if (rec.role === "child") throw codeError("parent");
  rec.mailingList = on === true;
  rec.updatedAt = new Date().toISOString();
  await saveRecord(rec);
  return publicProfile(rec);
}

export async function markTrialNotice(id) {
  const rec = await findProfileById(id);
  if (!rec || rec.trialNoticeAt) return publicProfile(rec);
  rec.trialNoticeAt = new Date().toISOString();
  rec.updatedAt = rec.trialNoticeAt;
  await saveRecord(rec);
  return publicProfile(rec);
}

/** Removes the live account. The log keeps only the suppression record. */
export async function deleteAccount(id, { action = "delete", locale = "" } = {}) {
  const rec = await findProfileById(id);
  if (!rec) throw codeError("unauthorized");
  const entry = deletionEntry(rec, action, locale);
  await rememberDeletion(entry);
  await forgetRecord(rec);
  return entry;
}
