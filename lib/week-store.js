...
import { readSealed } from "./sealed-bank.js";
...
function weeklyRel(locale) {
  const loc = normalizeLocale(locale);
  if (loc === "en") return "banks/weekly/current.json";
  return `banks/weekly/${loc}/current.json`;
}

function funnyRel(locale) {
  return `banks/gen-alpha/${normalizeLocale(locale)}.json`;
}

/** Weekly bank plus the Gen Alpha jokes Q&A keeps beside it. */
export function questionsForComedy(locale, pack) {
  const extra = (readSealed(funnyRel(locale)) || FUNNY[normalizeLocale(locale)])?.questions || [];
...
export function loadCurrentPack(locale = "en") {
  ...
  const bundled = readSealed(weeklyRel(loc)) || PACKS[loc] || PACKS.en;
...
export function applyReviewOverlay(pack, locale = "en") {
  ...
  const questions = (pack.questions || []).map((q) => ({
    ...q,
    status: reviewStatus(q, statuses),
  }));
...
export function publishToQuestions(pack, locale = "en") {
  ...
  const approved = {
    ...
    questions: (pack.questions || []).filter((q) => {
      const st = reviewStatus(q, bucket.statuses);
      return st !== "rejected";
    }),
  };
