...
/**
 * Two lockdown packs, each a different generation from the seat's own.
 * One step younger, then one step older.
 * Silent is the oldest pack, so it only moves younger: three generations, then four.
 */
export function lockdownPairGenerations(generation) {
  const g = normalizeGeneration(generation);
  const line = LOCKDOWN_LINE;
  if (g === "silent-generation") return ["gen-y", "gen-z"];
  const i = line.indexOf(g);
  if (i < 0) return ["gen-y", "gen-x"];
  const down = i + 1 < line.length ? line[i + 1] : line[i - 1];
  let up = i - 1 >= 0 ? line[i - 1] : line[Math.min(line.length - 1, i + 2)];
  if (up === down) {
    const further = i - 2 >= 0 ? line[i - 2] : line[Math.min(line.length - 1, i + 2)];
    up = further === down ? (line.find((name) => name !== down) || down) : further;
  }
  return [down, up];
}
...
function packsForLockdown(packs, avoid, difficultOnly = false) {
  const next = {};
  for (const g of GENERATIONS) {
    const pack = packs?.[g] || [];
    const fresh = pack.filter((q) => q && !avoid.has(String(q.id)) && isLockdownFactual(q));
    const difficult = fresh.filter((q) => q.tier === "difficult");
    const hard = fresh.filter((q) => HARD_TIERS.has(q.tier));
    const pool = ((difficultOnly && difficult.length ? difficult : (hard.length ? hard : fresh))).slice();
...
export function lockdownSet(players, packs, avoidIds = [], n = 5, opts = {}) {
  const which = Number.isInteger(opts.which) ? opts.which : null;
  const direction = which == null ? lockdownDirection(opts.refreshes) : (which % 2 === 0 ? "ahead" : "behind");
  ...
  const jumped = (players || []).map((p) => {
    ...
    let jump;
    if (which == null) {
      dest = lockdownJumpGeneration(home, direction);
      jump = home === "silent-generation" && direction === "behind" ? "back-more" : direction;
    } else {
      const pair = lockdownPairGenerations(home);
      dest = pair[which % 2] || pair[0];
      jump = home === "silent-generation"
        ? (which % 2 === 0 ? "down-3" : "down-4")
        : (which % 2 === 0 ? "down" : "up");
    }
    return {
      ...p,
  ...
  const filtered = dealRoundRobin(jumped, packsForLockdown(packs, avoid, which != null), n, {});
  if (filtered.questions.length >= n) return filtered.questions;
  if (which != null) {
    const hard = dealRoundRobin(jumped, packsForLockdown(packs, avoid, false), n, {});
    return hard.questions.length >= filtered.questions.length ? hard.questions : filtered.questions;
  }
  const factual = {};
