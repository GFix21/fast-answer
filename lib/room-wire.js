...
export function redactState(state) {
  ...
  return {
    ...
    mapClear: Array.isArray(src.mapClear) ? src.mapClear.map((id) => String(id || "")).filter(Boolean) : [],
    ...
    joinLeft: Number(src.joinLeft) || 0,
    armTv: Boolean(src.armTv),
...
function publicGuests(list) {
  return (list || []).map((guest) => {
    if (!guest || typeof guest !== "object") return guest;
    const { guestKey, profileId, ...rest } = guest;
    return rest;
  });
}

/** Room list and phone polls never receive the host key, guest keys, or upcoming answers. */
export function publicRoom(room, { host = false, guestKey = "" } = {}) {
  ...
  const out = {
    ...
    botPace: ["easy", "hard", "difficult", "extreme"].includes(room.botPace) ? room.botPace : "hard",
    ...
    playerCount: Math.min(12, Math.max(2, Number(room.playerCount) || 3)),
    topics: Array.isArray(room.topics) ? room.topics.map(String).slice(0, 40) : [],
    screen: room.screen === "tv" ? "tv" : "off",
    guests: publicGuests(room.guests),
    ...
    tvSeenAt: Number(room.tvSeenAt) || 0,
    hostGeneration: String(room.hostGeneration || "").slice(0, 40),
    hostAge: room.hostAge ?? "",
    hostAgeBracket: String(room.hostAgeBracket || "").slice(0, 8),
  };
  if (host && Array.isArray(room.refreshQs)) out.refreshQs = room.refreshQs;
  if (guestKey) out.guestKey = guestKey;
  return out;
}
