...
const BUNDLED = {
  ...
  "fr-CA": require("../banks/placement/fr-CA/generational-first-pass.json"),
...
export function loadPlacementPack(locale = "en") {
  const loc = normalizeLocale(locale);
  const b = bucket(loc);
  const bundled = readSealed(placementRel(loc)) || BUNDLED[loc] || BUNDLED.en;
