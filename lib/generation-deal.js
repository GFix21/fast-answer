...
function takeJokes(pool, jokeNeed, recent, renegadesLeft) {
  const renegades = pool.filter((q) => q.structure === "renegade-joke");
  const regular = pool.filter((q) => q.funny === true && q.structure !== "renegade-joke");
  const jokes = [];
  if (renegadesLeft > 0 && jokeNeed > 0 && renegades.length) {
    jokes.push(...takeSpread(renegades, 1, recent));
  }
  jokes.push(...takeSpread(regular, jokeNeed - jokes.length, recent));
  return jokes;
}
...
export function dealShow(questions, avoid = []) {
  ...
  for (const [tier, need] of Object.entries(SHOW_DEAL)) {
    ...
    renegadesLeft -= jokes.filter((q) => q.structure === "renegade-joke").length;
