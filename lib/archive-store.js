...
export function loadArchivedWeek(monthKey, weekKey) {
  ...
  const base = g.__faFlowArchive.weeks[key]
    ? structuredClone(g.__faFlowArchive.weeks[key])
    : (weeks[key] ? structuredClone(weeks[key]) : null);
  if (!base) return null;
  const swaps = g.__faArchiveSwaps?.[key];
...
export function swapArchivedQuestion(monthKey, weekKey, oldId, nextQ) {
  const key = `${monthKey}/${weekKey}`;
  if (!g.__faArchiveSwaps) g.__faArchiveSwaps = {};
  if (!g.__faArchiveSwaps[key]) g.__faArchiveSwaps[key] = {};
  g.__faArchiveSwaps[key][String(oldId)] = nextQ;
  return loadArchivedWeek(monthKey, weekKey);
}
