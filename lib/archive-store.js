...
function bundled(locale) {
  const loc = normalizeFlowLocale(locale);
  const base = byLocale[loc] || { registry: { months: [], updatedAt: null }, indexes: {}, weeks: {} };
  const weeks = {};
  for (const [key, pack] of Object.entries(base.weeks || {})) {
    const prefix = loc === "en" ? "banks/archive" : `banks/archive/${loc}`;
    weeks[key] = readSealed(`${prefix}/${key}.json`) || pack;
  }
  return { ...base, weeks };
}
