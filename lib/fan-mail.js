...
import { sendHouseMail } from "./house-mail.js";
import { forwardToGmgList } from "./gmg-list.js";
...
export async function deployWelcome({ email, name, locale }) {
  const state = await load();
  const letter = publicLetter(state);
  state.metrics.mailingJoins += 1;
  let mailed = false;
  try {
    const remote = await forwardToGmgList({ email, name, locale });
    if (remote.skipped) {
      ...
      mailed = Boolean(remote.ok && (remote.emailed || remote.duplicate));
    }
  } catch {
    mailed = false;
  }
