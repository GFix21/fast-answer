...
export async function listRejectLog(filter = {}) {
  ...
  if (filter.locale) {
    const loc = String(filter.locale).toLowerCase().slice(0, 2);
    entries = entries.filter((e) => !e.locale || e.locale === loc);
  }
...
export async function markRegenerated(questionId, regeneratedQuestionId, locale) {
  const doc = await loadDoc();
  const loc = locale ? String(locale).toLowerCase().slice(0, 2) : null;
  let hit = null;
  const entries = (doc.entries || []).map((e) => {
    ...
    if (!hit) {
      hit = { ...e, regeneratedQuestionId, regeneratedAt: new Date().toISOString() };
      return hit;
    }
    return e;
  });
