import { dealRamp } from "./generation-deal.js";
import { orderShowSets } from "./show-pace.js";
...
export function buildDeviceCohort(weekly, placement, locale, now = new Date().toISOString()) {
  ...
  for (let i = 0; i < COHORT_SHOW_DECKS; i++) {
    const deck = orderShowSets(dealRamp(pool, { seats: [] }));
  ...
  return {
    ...
    ages: summary.ages,
    sendsFor: summary.sendsFor,
    playable: summary.playable,
    ageYear: summary.year,
    target: PACK_TARGET,
    held: summary.held,
    lockdownRefreshes: 0,
    packCursors: {},
