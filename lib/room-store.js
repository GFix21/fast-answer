/**
 * Open TV rooms.
 * When Upstash is configured (FAST_ANSWER_REDIS_KV_REST_API_URL + TOKEN),
 * every Vercel instance reads and writes the same room.
 * Without those keys, files and the in-memory map cover one machine,
 * and Vercel Runtime Cache is used as a fallback.
 */
...
function redis() {
  ...
  if (!cfg) return null;
  if (!redisClient) redisClient = new Redis(cfg);
  return redisClient;
}
