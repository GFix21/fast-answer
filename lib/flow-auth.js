 ...
 * Bootstrap password matches GMGbrand Flow: env FLOW_PASSWORD, else the
 * house fallback. NFC-normalize at compare time.
 */
import { createHmac, timingSafeEqual } from "node:crypto";

export const FLOW_COOKIE = "fa_flow_session";
const MAX_AGE_SEC = 60 * 30; // 30 minutes soft ceiling
/** Same bootstrap as GMGbrand Flow (D zero zero r, percent, euro). */
const FLOW_BOOTSTRAP = "D00r%€";

export function getFlowPassword() {
  const fromEnv = process.env.FLOW_PASSWORD?.trim();
  if (fromEnv) return fromEnv;
  return FLOW_BOOTSTRAP;
}
